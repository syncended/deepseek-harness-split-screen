...
`;
    ...
    const COPY = {
      en: {
        ...
        markdownLabels: { code: { copyLabel: "Copy", copiedLabel: "Copied" }, footnotes: "Footnotes" },
      ...
      ru: {
        ...
        markdownLabels: { code: { copyLabel: "Копировать", copiedLabel: "Скопировано" }, footnotes: "Сноски" },
      ...
      zh: {
        ...
        markdownLabels: { code: { copyLabel: "复制", copiedLabel: "已复制" }, footnotes: "脚注" },
    ...
    function useConversationSnapshot(uiConversation, session, snapshot) {
      // New DSH separates lifecycle/control state from the Chat target. Keep the
      // legacy Session snapshot path when that optional assembly is unavailable.
      const binding = session && uiConversation && typeof uiConversation.binding === "function"
        ? uiConversation.binding(session.sessionId) : null;
      const source = binding && typeof binding.target === "function" ? binding.target("chat") : null;
      const chat = useObservable(source, undefined);
      // Merge after reading the stable external snapshot, never in getSnapshot.
      return chat ? { ...snapshot, ...chat.legacy, chat } : snapshot;
    }
    ...
    function AssistantMessage({ row, streaming, activeCopy }) {
      const labels = (activeCopy || COPY.en).markdownLabels;
      if (!Array.isArray(row.blocks)) return row.interrupted ? h(StoppedBadge, { row }) : h(MarkdownText, { text: row.text, streaming, labels });
      const visible = row.blocks.filter((block) => block && (block.kind === "text" || block.kind === "reasoning" || block.kind === "image"));
      if (visible.length === 0) return row.interrupted ? h(StoppedBadge, { row }) : h(MarkdownText, { text: row.text, streaming, labels });
      const rendered = visible.map((block, index) => {
        ...
        return h(MarkdownText, { text: block.text || "", streaming: streaming && index === visible.length - 1, labels, key: `text-${index}` });
      });
    ...
    function MessageRow({ row, streaming = false, activeCopy }) {
      ...
      if (row.role === "user") return h("div", { className: "dsh-split-message", "data-role": row.role }, MessageText ? h(MessageText, { text: row.text }) : row.text);
      if (row.role === "assistant") return h("div", { className: `dsh-split-message${streaming ? " dsh-split-stream" : ""}`, "data-role": row.role }, h(AssistantMessage, { row, streaming, activeCopy }));
    ...
    function PaneView(props) {
      const { node, list, workspaces, sessions, uiConversation, modelDirectories, active, paneCount, onActive, onSplit, onRemove, onReset, onSwap, onOpenMain, onTabSelect, onTabClose, onTabAdd, activeCopy } = props;
      ...
      const { session, snapshot: lifecycle } = useSessionSnapshot(sessions, sessionId);
      const snapshot = useConversationSnapshot(uiConversation, session, lifecycle);
    ...
    function equalPaneProps(previous, next) {
      if (
        ...
        previous.uiConversation !== next.uiConversation ||
    ...
    function SplitScreenEntry({ sessions, workspaces, uiConversation, modelDirectories, locale, sessionId, onExit }) {
      const localeSnapshot = useObservable(locale, EMPTY_LOCALE);
      const activeCopy = copy(localeSnapshot.active);
    ...
    function apply(ctx) {
      ...
      // DSH renamed the resident conversation slot; wait for either declaration.
      const centerSlots = new Set();
      let mountedSlot = null;
      let disposeCenter = null;
      const listeners = new Set();
      const notify = () => { for (const listener of listeners) listener(); };
      const mountCenter = () => {
        const slot = ["main.conversation", "conversation"].find((name) => centerSlots.has(name));
        if (disposeCenter && (!active || mountedSlot !== slot)) unmountCenter();
        if (!slot || !active || disposeCenter) return;
        try {
          mountedSlot = slot;
          disposeCenter = ctx.slots.register(
            { name: slot, priority: -100 },
            (props) => h(SplitScreenEntry, {
              ...props,
              sessions: ctx.sessions,
              workspaces: ctx.workspaces,
              uiConversation: typeof ctx.get === "function" ? ctx.get("uiConversation") : undefined,
      ...
      for (const slot of ["main.conversation", "conversation"]) {
        ctx.effect(
          () => ctx.slots.inject(slot, () => {
            centerSlots.add(slot);
            mountCenter();
            return () => { centerSlots.delete(slot); mountCenter(); };
          }),
          `split-screen: center workspace (${slot})`,
        );
      }
      ctx.effect(
      ...
      ctx.effect(() => () => { active = false; unmountCenter(); listeners.clear(); }, "split-screen: cleanup");
    }
